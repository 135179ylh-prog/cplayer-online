...
if "const PLAY_MODES = ['sequence', 'repeat_one', 'shuffle'];" not in c:
    # insert after first occurrence of 'playMode' usage area in script: find 'let currentIndex'
    anchor = "let currentIndex"
    i = c.find(anchor)
    if i < 0:
        # fallback: after 'use strict' in main script
        anchor = "'use strict';"
        i = c.find(anchor)
    if i < 0:
        raise SystemExit("no anchor for play mode logic")
    line_end = c.find("\n", i)
    c = c[:line_end] + "\n" + logic + c[line_end:]
    print("play mode logic inserted")

# init saved mode on boot
if "cp_play_mode" in c and "updatePlayModeUI();" not in c:
...
s = re.sub(r"const CACHE_NAME = '[^']+'", "const CACHE_NAME = 'cplayer5-v31-playmode-fix2'", s, count=1)
